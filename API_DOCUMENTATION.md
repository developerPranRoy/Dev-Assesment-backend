...
`GET /auth/google` — no auth, open in a browser

Redirects to Google's consent screen. Not callable from Postman in any meaningful way — it needs to complete a real Google login form.

`GET /auth/google/callback?code=...` — Google redirects here automatically

Exchanges the code for the user's Google profile, then:

- If a user with that `googleId` already exists → logs them in.
- Else if a user with that email already exists (registered via password) → links the Google account to it (adds `googleId`, keeps their existing role).
- Else → creates a new user with role `CANDIDATE` (the OAuth flow doesn't collect a role — `COMPANY` accounts register via email/password instead).

**200** → `{ user, accessToken, refreshToken }`, same shape as a normal login. **501** if `GOOGLE_CLIENT_ID` isn't configured. **400** if Google reports an error (e.g. the user cancelled consent) or the callback is missing its `code`.
...
`GET /companies/:id` — auth required (any role)

**200** → company details plus member list (`userId`, `permissionLevel` per member). **403** if the requester is not a member of the company (admins can view any company).
...
Assessment must be `PUBLISHED`. Each invitation gets a random token and a 7-day expiry. **201** → array of invitations. Each invited candidate receives an email with a direct link to accept the invitation. If `SMTP_HOST` is not configured, emails are skipped but invitations are still created — the token is returned in the response for manual sharing.
